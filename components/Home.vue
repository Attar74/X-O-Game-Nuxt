<template>
  <div class="p-[2rem] lg:py-[1rem]">
		...
		<p class="text-4xl md:text-5xl mb-[2rem] text-center" v-else-if="!GameDone">
			...
			<span
				class="text-6xl"
				:class="[isX ? 'text-[#09C372]' : 'text-[#498AFB]']"
			>
				{{ isX ? 'X' : 'O' }}
			</span>
			's turn
		...
		<div class="flex justify-center" v-for="j in 3" :key="j">
			...
			>
				<p
					class="text-[3rem] md:text-[5rem] mx-auto my-auto"
					:class="[
						game[j - 1][i - 1] == 'x' ? 'text-[#09C372]' : 'text-[#498AFB]',
					]"
				>
					{{ game[j - 1][i - 1] }}
				</p>
		...
		<div class="flex">
			<button
				@click="reset"
				class="bg-[#FF3860] text-[#fff] px-10 py-2 rounded-md mx-auto mt-[2rem] hover:bg-[#ff3860d9]"
			>
				Reset
			</button>
		</div>
...
const doIt = (j: number, i: number) => {
  ...
  game[j - 1][i - 1] = game[j - 1][i - 1].length
    ? game[j - 1][i - 1]
    : isX.value
    ? 'x'
    : 'o';
